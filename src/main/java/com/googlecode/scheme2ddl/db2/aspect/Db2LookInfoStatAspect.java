...
public class Db2LookInfoStatAspect {
    ...
    private List<Db2LookInfo> listExludedByAlterSequenceFilter;

    public Db2LookInfoStatAspect() {
        this.listExludedByFilter = Collections.synchronizedList(new ArrayList<Db2LookInfo>());
        this.listExludedByAlterSequenceFilter = Collections.synchronizedList(new ArrayList<Db2LookInfo>());
    }
    ...
    @AfterReturning(pointcut = "execution(boolean com.googlecode.scheme2ddl.db2.IsAlterSequenceValuesPredicate.evaluate(..)) &&"
            + "args(object)",
            returning = "retVal")
    public void exludedByFilterIsAlterSequenceV(Object object, boolean retVal) {
        if (retVal == false ) {
            listExludedByAlterSequenceFilter.add((Db2LookInfo) object);
        }
    }
    ...
    public void printStatistic(Job job, JobParameters jobParameters) {
        ...
        if (!listExludedByFilter.isEmpty() || !listExludedByAlterSequenceFilter.isEmpty()) {
    ...
    public void prettyPrint(String schemaName) {
        ...
        if (!listExludedByFilter.isEmpty()){
            sb.append(String.format("%d alter table statements with auto generated sequence values was filtered", listExludedByFilter.size()));
            sb.append(lSep);
        }
        if (!listExludedByAlterSequenceFilter.isEmpty()) {
            sb.append(String.format("%d alter sequence statements with actual sequence values was filtered", listExludedByAlterSequenceFilter.size()));
            sb.append(lSep);
        }
        sb.append(lSep);
