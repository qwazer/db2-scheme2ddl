...
public class UserObjectProcessor implements ItemProcessor<UserObject, UserObject>, InitializingBean {
    ...
    @Value( value = "#{mainProperties[filterSequenceValues]}")
    private boolean filterSequenceValues;
    @Autowired
    private Predicate isAlterTableAutoGeneratedPredicate;
    @Autowired
    private Predicate isAlterSequenceValuesPredicate;

    @Autowired
    private Comparator db2LookInfoComparator;

    @Autowired
    private Properties mainProperties;  //keep it for overwriting from cli invokation
    ...
    private String map2Ddl(UserObject userObject) throws CannotGetDDLException, NonSkippableException {
        try {
            ...
            if (filterSequenceValues) {
                ...
                CollectionUtils.filter(list, isAlterSequenceValuesPredicate);
            }
            Collections.sort(list, db2LookInfoComparator);
    ...
    public void afterPropertiesSet() throws Exception {
        stopOnWarning = Boolean.valueOf(mainProperties.getProperty("stopOnWarning"));
        filterSequenceValues = Boolean.valueOf(mainProperties.getProperty("filterSequenceValues"));
    }
}
